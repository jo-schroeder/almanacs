...
# Making Tables
master_test <- fread("./_data/master.csv.gz") %>% mutate(across(everything(), as.character)) %>% filter(!(file_id %in% new_files$id)) %>% bind_rows(master_new)

fwrite(master_new, "./_data/master.csv.gz")
