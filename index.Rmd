...
# Making Tables
master <- read_csv('./_data/master.csv.gz') %>% mutate(across(everything(), as.character)) %>% filter(!(file_id %in% new_files$id)) %>% bind_rows(master_new)

write_csv(master_new, "./_data/master.csv.gz")
