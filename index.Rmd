...
# Authenticate through Actions
certificate <- Sys.getenv("CERTIFICATE_BASE64")
decoded_cert <- base64_decode(certificate)

# Save the decoded certificate as a file
cert_file <- ".secrets/token.json"
dir.create(".secrets", showWarnings = FALSE) # Ensure the .secrets directory exists
writeBin(decoded_cert, cert_file)

# Authenticate with Google Sheets and Drive using the decoded certificate
gs4_auth(path = cert_file, email = "schroejh@bc.edu", scopes = "https://www.googleapis.com/auth/drive")
drive_auth(path = cert_file)
...
print(time2-time1)
master_new <- master %>% select(1:31)
```

```{r, echo = FALSE, warning = FALSE, message = FALSE}
# Making Tables
master <- read_csv('./_data/master.csv') %>% mutate(across(everything(), as.character)) %>% filter(!(file_id %in% new_files$id)) %>% bind_rows(master_new)

write_csv(master, "./_data/master.csv")
...
datatable(master_inst)
datatable(master_geo)
datatable(master_att)
datatable(master_pers)
