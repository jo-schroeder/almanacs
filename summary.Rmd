...
async function sendDataToGitHub() {
  ...
  fetch(apiUrl, {
    method: "POST",
    headers: {
      "Authorization": `token ${GH_TOKEN}`, // Use GH_TOKEN secret from repository
...
numerous_people <- master %>% select(persID, persName, diocese, year) %>% 
  ...
  mutate(Select = sprintf('<input type="checkbox" class="rowCheckbox">'))

# Create the DataTable with a "Save to GitHub" button
datatable(
  numerous_people,
  rownames = FALSE,
  escape = FALSE,
  options = list(
    ...
    buttons = list(
      list(
        text = "Save to GitHub",
        action = JS("sendDataToGitHub()")  # Calls the JavaScript function
      )
    ),
