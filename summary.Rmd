...
next_available_id <- master %>% select(instID, diocese) %>% 
  ...
  select(diocese, id_no)

datatable(next_available_id, filter = list(position = 'top', clear = FALSE))
```

```{r, echo = FALSE, warning = FALSE, message = FALSE}
numerous_people <- master %>% select(persID, persName, diocese, year) %>% 
  filter(!is.na(persID), !is.na(persName)) %>%
  mutate(id_no = as.numeric(str_extract(persID, "[:digit:]{3}"))) %>%
  filter(id_no >= 3) %>% arrange(persName) %>%
  select(persID, persName, diocese, year)

datatable(numerous_people, filter = list(position = 'top', clear = FALSE))
