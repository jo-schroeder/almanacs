...
K_initialID <- data.frame(form = '=ARRAYFORMULA(
  SUBSTITUTE(
    IF( 
      REGEXMATCH(A2:A, "(Sister|Mother|OtherException|Brother|Bro.|Sœur|Soeur)") +
      REGEXMATCH(B2:B, "(Brother|Sister|Ladies|Sœur|Soeur)") +
      (ISBLANK(I2:I) * ISBLANK(G2:G)),
      "",
      IF(
        ISBLANK(I2:I),
...
#for(i in 1:nrow(files)){
  ...
  df <- bind_cols(df, colnames(peopleInfo)) 
  #}
  
colnames(df) <- files$name
```
