...
info <- NULL
    ...
    for(i in 1:nrow(files)){
      ...
      F2 <- range_read_cells(files$id[i], 
                sheet = "peopleInfo",
                cell_data = "full",
                range = "F2")
      ...
      info_sheet <- F2[[4]][[1]][["userEnteredValue"]][["formulaValue"]]
...
for(i in 1:nrow(files)){
    ...
    F_persName_clean <-
      '=ARRAYFORMULA(LOWER(REGEXREPLACE(REGEXREPLACE(REGEXREPLACE(B2:B, "[[:punct:]-—]", ""), "\\b(Von|De|La|Der|Van|von|de|la|der|van|Du|Da|du|da)\\s+(\\w+)", "$1-$2"), "^\\s", "")))'
...
```{r}
info <- info %>% pivot_longer(everything()) 

info %>% filter(str_detect(value, '"^\\s", ""'))
```
