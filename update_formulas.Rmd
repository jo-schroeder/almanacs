...
for(i in 186:nrow(files)){
  ...
  master <- range_read(files$id[i],
               ...
               range = "A:A") %>%
              rownames_to_column()
  colnames(master) <- c("rowname", "master")
 
  instLocations <- range_read(files$id[i],
               sheet = "instLocations",
               range = "A:A") %>%
              rownames_to_column()
  colnames(instLocations) <- c("rowname", "instLocations")

  df <- master %>% full_join(instLocations, by = "rowname")
  df$name <- files$name[i]
  
  df %>% mutate(master = as.character(master), 
                instLocations = as.character(instLocations))
  
  info <- bind_rows(info, df)
 
  }
 
test2 <- info %>%
  filter(master != instLocations)

test <- test2 %>% select(rowname, name) %>% group_by(name) %>% slice_min(rowname)
```
